/**
 * 
 * Self Description JSON Schema v1.1.0
 * 
 ...
 * (c) Copyright 2018-2019 W3plan Technologies, https://www.w3plan.net/
 */
...
/**
 * Print message on the browser console
 *
 * Static method
 * 
 * @private 
 * @param {integer} errorLevel  Defined error level from 0 to 3
...
/**
 * Self-Description JSON Schema Module
 * 
 * Supermodule
 *
 * @constructor
...
var sdjsmd = function() {
  ...
  var validateCstr = function(cstr, name, val) {
    ...
    if (flags.length > 0 && flags.indexOf(0) === -1) {
      ...
      errorHandle.printMessage(2, "The value " + val + " of member " + name + " againsts constraint " + JSON.stringify(cstr));
      return false;
    }
  };
  ...
  var validateType = function(type, name, val) {
    ...
    switch(type) {
      ...
      case "jsonString":
        try {
          ...
          errorHandle.printMessage(2, val + " againsts type " + type);
          vflag = false;
        }
        vflag = true;
        break;
      ...
      case "base64":
        try {
          ...
          errorHandle.printMessage(2, val + " against type " + type);
          vflag = false;
        }
        vflag = true;
        break;
    ...
    if (vflag) {
      ...
      errorHandle.printMessage(2, "The value " + val + " of member " + name + " againsts type " + type);
      return false;
    }
  };
  
  /**
   * Validate JSON object with Self-Described JSON Schema
   * 
   * @public 
   * @param {object} jdata      JSON object to validate with SDJS  
   * @param {object|null} glb   Global SDJS
   * @return {undefined}
   */
 ...
 *
 */
this.sdjs = new sdjsApp("Version 1.1.0");
...
/**
 * Return true if all elements in an array pass a test function
 *
 */
