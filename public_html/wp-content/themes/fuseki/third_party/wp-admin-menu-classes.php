...
class WP_AdminMenuSection {
	...
	function add_item($item,$args=array()) {
		...
		if (is_a($item,'WP_AdminMenuItem')) {
			...
			$item['hookname'] = add_theme_page( $slug, $item['page_title'], $item['title'], $item['capability'], $item['slug'], $item['function']);
